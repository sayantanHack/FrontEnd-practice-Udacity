...
var navList, fst , link;
 
navList = $(".nav-list");         //selecting the nav-list class from the html code using jQuery($).
fst = navList.children().first();    // getting into its childrens(li) and also selecting the first one through first() method of jQuery.
link = fst.find('a');              // then find the a tag from there
link.attr("href","#1");             //then to add the href and its value I used the attr() means attribute method.
